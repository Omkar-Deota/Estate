

function Owntab() {
  return (
    <div>
      <div className="flex flex-wrap justify-center gap-4">
            <div className="flex flex-col max-w-xs bg-white border border-gray-200 rounded-lg shadow-md lg:h-96">
              <img
                className="w-24 h-24 object-cover rounded-full mx-auto mt-4"
                src="src/components/graphics/Screenshot 2024-08-10 193221.png"
                alt="Image 2"
              />
              <div className="flex-1 p-4 text-center">
                <h2 className="text-lg font-semibold text-gray-800">Card 2</h2>
                <p className="text-gray-600 mt-2">
                Paragraphs are the building blocks of papers. Many students define paragraphs in terms of length:
                  a paragraph is a group of at least five sentences, a paragraph is half a page long, etc. In reality,
                  though, the unity and coherence of ideas among sentences is what constitutes a paragraph.
                </p>
                <button className="bg-pink-600 rounded-xl p-2 hover:bg-pink-700 mt-2">Book Now</button>
              </div>
            </div>

            <div className="flex flex-col max-w-xs bg-white border border-gray-200 rounded-lg shadow-md lg:h-96">
              <img
                className="w-24 h-24 object-cover rounded-full mx-auto mt-4"
                src="src/components/graphics/Screenshot 2024-08-10 193229.png"
                alt="Image 2"
              />
              <div className="flex-1 p-4 text-center">
                <h2 className="text-lg font-semibold text-gray-800">Card 2</h2>
                <p className="text-gray-600 mt-2">
                  Paragraphs are the building blocks of papers. Many students define paragraphs in terms of length:
                  a paragraph is a group of at least five sentences, a paragraph is half a page long, etc. In reality,
                  though, the unity and coherence of ideas among sentences is what constitutes a paragraph.
                </p>
                <button className="bg-pink-600 rounded-xl p-2 hover:bg-pink-700 mt-2">Book Now</button>
              </div>
            </div>

            <div className="flex flex-col max-w-xs bg-white border border-gray-200 rounded-lg shadow-md lg:h-96">
              <img
                className="w-24 h-24 object-cover rounded-full mx-auto mt-4"
                src="src/components/graphics/Screenshot 2024-08-10 193241.png"
                alt="Image 2"
              />
              <div className="flex-1 p-4 text-center">
                <h2 className="text-lg font-semibold text-gray-800">Card 2</h2>
                <p className="text-gray-600 mt-2">
                  Paragraphs are the building blocks of papers. Many students define paragraphs in terms of length:
                  a paragraph is a group of at least five sentences, a paragraph is half a page long, etc. In reality,
                  though, the unity and coherence of ideas among sentences is what constitutes a paragraph.
                </p>
                <button className="bg-pink-600 rounded-xl p-2 hover:bg-pink-700 mt-2">Book Now</button>
              </div>
            </div>

            <div className="flex flex-col max-w-xs bg-white border border-gray-200 rounded-lg shadow-md lg:h-96">
              <img
                className="w-24 h-24 object-cover rounded-full mx-auto mt-4"
                src="src/components/graphics/Screenshot 2024-08-10 193319.png"
                alt="Image 2"
              />
              <div className="flex-1 p-4 text-center">
                <h2 className="text-lg font-semibold text-gray-800">Card 2</h2>
                <p className="text-gray-600 mt-2">
                  Paragraphs are the building blocks of papers. Many students define paragraphs in terms of length:
                  a paragraph is a group of at least five sentences, a paragraph is half a page long, etc. In reality,
                  though, the unity and coherence of ideas among sentences is what constitutes a paragraph.
                </p>
                <button className="bg-pink-600 rounded-xl p-2 hover:bg-pink-700 mt-2">Book Now</button>
              </div>
            </div>

            <div className="flex flex-col max-w-xs bg-white border border-gray-200 rounded-lg shadow-md lg:h-96">
              <img
                className="w-24 h-24 object-cover rounded-full mx-auto mt-4"
                src="src/components/graphics/Screenshot 2024-08-10 193334.png"
                alt="Image 2"
              />
              <div className="flex-1 p-4 text-center">
                <h2 className="text-lg font-semibold text-gray-800">Card 2</h2>
                <p className="text-gray-600 mt-2">
                  Paragraphs are the building blocks of papers. Many students define paragraphs in terms of length:
                  a paragraph is a group of at least five sentences, a paragraph is half a page long, etc. In reality,
                  though, the unity and coherence of ideas among sentences is what constitutes a paragraph.
                </p>
                <button className="bg-pink-600 rounded-xl p-2 hover:bg-pink-700 mt-2">Book Now</button>
              </div>
            </div>

            <div className="flex flex-col max-w-xs bg-white border border-gray-200 rounded-lg shadow-md lg:h-96">
              <img
                className="w-24 h-24 object-cover rounded-full mx-auto mt-4"
                src="src/components/graphics/Screenshot 2024-08-10 193352.png"
                alt="Image 2"
              />
              <div className="flex-1 p-4 text-center">
                <h2 className="text-lg font-semibold text-gray-800">Card 2</h2>
                <p className="text-gray-600 mt-2">
                  Paragraphs are the building blocks of papers. Many students define paragraphs in terms of length:
                  a paragraph is a group of at least five sentences, a paragraph is half a page long, etc. In reality,
                  though, the unity and coherence of ideas among sentences is what constitutes a paragraph.
                </p>
                <button className="bg-pink-600 rounded-xl p-2 hover:bg-pink-700 mt-2">Book Now</button>
              </div>
            </div>


            <div className="flex flex-col max-w-xs bg-white border border-gray-200 rounded-lg shadow-md lg:h-96">
              <img
                className="w-24 h-24 object-cover rounded-full mx-auto mt-4"
                src="src/components/graphics/Screenshot 2024-08-10 193415.png"
                alt="Image 2"
              />
              <div className="flex-1 p-4 text-center">
                <h2 className="text-lg font-semibold text-gray-800">Card 2</h2>
                <p className="text-gray-600 mt-2">
                  Paragraphs are the building blocks of papers. Many students define paragraphs in terms of length:
                  a paragraph is a group of at least five sentences, a paragraph is half a page long, etc. In reality,
                  though, the unity and coherence of ideas among sentences is what constitutes a paragraph.
                </p>
                <button className="bg-pink-600 rounded-xl p-2 hover:bg-pink-700 mt-2">Book Now</button>
              </div>
            </div>
          </div>

    </div>
  )
}

export default Owntab